...
# Logic was extracted from OCEANSIM Package and updated for custom purposes
...
class custom_keyboard_cmd:
    def __init__(self,
                 ...
                 input_keyboard_mapping: dict = {
                    ...
                    toggle_command: int = int(1.0),
                    toggle_keyboard_mapping: dict = {
                        # Tab command
                        "T": int(-1.0)
                    }
                ) -> None:
        ...
        self._toggle_keyboard_mapping = toggle_keyboard_mapping
    ...
    def _sub_keyboard_event(self, event, *args, **kwargs) -> bool:
        ...
        if event.type == carb.input.KeyboardEventType.KEY_PRESS:
            ...
            if event.input.name in self._input_keyboard_mapping:
                self._base_command -= np.array(self._input_keyboard_mapping[event.input.name])
            # toggle logic
            elif event.input.name in self._toggle_keyboard_mapping:
                self._toggle_command *= int(self._toggle_keyboard_mapping[event.input.name])
        return True
